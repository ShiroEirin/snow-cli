...
latest_true_state:
  - 2026-04-08 已补记一条新的边界审计纠偏：此前使用旧结论的审查把 `subAgentExecutor` 旁路、`chat.ts` 硬编码请求头与 “Core 直连 translator” 误当成当前事实；按当前分支 `e79f3fb` 后代码重新核对，这三条都不应再作为现状风险描述。其中 `subAgentExecutor.ts` 已统一走 `prepareToolPlane()`，`chat.ts` 当前通过 `resolveVcpRequestHeaders()` 注入 `x-snow-*` 头，`bridgeManifestTranslator.ts` 也未被 `conversation/core` 直接导入。
  - 同次已把当前真实债务措辞收敛为 4 条：`conversationSetup.ts` 直接依赖 `toolPlaneFacade` 并回传 `toolPlaneKey/runtimeState`，属于受控 seam 泄漏；`streamFactory.ts` 位于对话流入口并统一经过 `resolveVcpModeRequest()` + `applyVcpOutboundMessageTransforms()`，但 native / non-chat 路径会短路为 no-op，不应夸大成“无条件 VCP 变换”；`toolExecutor.ts` 当前是消费 execution binding 后按 `binding.kind` 分发 local/bridge，不是执行层重新做路由决策；`apiConfig.ts` 继续把 `backendMode/toolTransport/bridgeWsUrl/bridgeVcpKey` 等 VCP 语义与通用 API 配置混放，仍是当前最明确的配置污染点。
  - 2026-04-08 基于现代码的继续复核还确认了 3 条仍需保留的真实收口项：`contextCompressor.ts` 仍直接依赖 `vcpCompatibility/display.ts` 与 `mode.ts`，`assistantContentSanitizer.ts` 仍直接依赖 `stripVcpDisplayBlocks()`，`teamExecutor.ts` 的 synthetic teammate tools 仍在执行器内部注入而未前移到 tool-plane/router seam。它们都属于“过渡基线允许存在、但后续应继续收口”的边界债务，不构成当前必须回滚的失控级问题。
  - 2026-04-08 当日晚间已完成一轮新的最小边界收口，并经主窗口复审确认没有偏离五层过渡基线：`apiConfig.ts` 现已拆出 `ApiCoreConfig + VcpApiConfig` 与 `splitApiConfig()`，但继续保持 `snowcfg` 扁平落盘；`conversationSetup.ts` 已把 `toolSnapshotKey + toolPlaneRuntimeState` 收束到 `toolContext`，同时让 `conversationTypes/useStreamingState` 改为经由 `conversationSetup` 取 runtime-state 类型；`contextCompressor.ts` 与 `assistantContentSanitizer.ts` 已改为只依赖新的薄适配层 `source/utils/core/vcpCompatibilityAdapter.ts`；`teamExecutor.ts` 的 teammate synthetic tools 也已前移到 `toolPlaneFacade.ts` 的 `syntheticTools` 注入口，由新增 `teammateSyntheticTools.ts` 集中定义。主窗口扩大复审后未发现新的 `Critical/Warning`，残余仅为已知非阻断债务：`conversationSetup` 仍直接依赖 facade，`StatusLine/ChatFooter` 等 UI 仍直接使用 `PreparedToolPlane['runtimeState']`，以及 synthetic tool 的实际执行分发仍在 `teamExecutor` 私有分支。
  - 2026-04-08 同次定向验证已通过：`npx ava source/utils/config/apiConfig.test.ts source/ui/pages/configScreen/configDraft.test.ts source/utils/session/vcpCompatibility/bridgeClient.test.ts source/utils/session/vcpCompatibility/toolPlaneFacade.test.ts source/utils/session/vcpCompatibility/toolRouteArbiter.test.ts source/utils/session/vcpCompatibility/conversationSetupSeam.test.ts source/hooks/conversation/useConversation.test.ts source/utils/core/contextCompressor.test.ts source/utils/core/vcpCompatibilityAdapter.test.ts source/hooks/conversation/utils/assistantContentSanitizer.test.ts source/utils/execution/teamExecutor.test.ts` = `53 passed`，`npm run build:ts -- --pretty false` 通过，`git diff --check` 通过。
  - 2026-04-08 黑盒复测也已补做并通过，且要明确纠偏：首次 fresh 复跑时 `core/team local` 都在 “SSE session created 后无后续事件” 阶段超时，根因是用户侧 `VCPToolBox/VCP` 当时未启动，不应记成本轮代码回归；在用户补启动后重新执行 `node scripts/vcp-blackbox.mjs --config scripts/fixtures/vcp-blackbox.local-bridge.json --modes local,bridge,hybrid` 与 `node scripts/vcp-blackbox.mjs --suite team --config scripts/fixtures/vcp-blackbox.local-bridge.json --mode local` 均已通过，fresh session 分别为 `7a22f005-a24a-4660-9c28-fcdfe60d8c99`（core local）、`51130f5c-9e9c-4928-a7cc-0dab831a2245`（core bridge）、`3ad1a468-32a7-4c9f-90a0-7be060e25f9d`（core hybrid）、`a73946be-87ef-41d9-a1ff-720cab73dd52`（team local）。当前 fresh 样本未见本轮 seam 收口引入 tool-plane 误路由、assistant 净化回退或 teammate synthetic tool 暴露异常。
  - 2026-04-08 已完成 `snow-cli` 当前分支对官方 `v0.7.7` 的合并并推送远端，commit=`7755573`；同次已按 Kimi 审计做最小边界修补：`source/utils/execution/subAgentExecutor.ts` 不再旁路 `collectAllMCPTools()`，而是统一经 `prepareToolPlane()` 取工具面；`source/utils/acp/acpManager.ts` 也已切到 `prepareToolPlane()` + `toolPlaneKey` 执行，并在 prompt/stop 清理 session 级 binding/snapshot；`source/utils/session/vcpCompatibility/toolExecutionBinding.ts` 的无 key fallback 现只再暴露 default plane，不再把 session-scoped binding 漏给全局调用。对应定向验证已通过：`npx ava source/utils/session/vcpCompatibility/toolExecutionBinding.test.ts source/utils/execution/subAgentExecutor.test.ts source/utils/session/vcpCompatibility/toolPlaneFacade.test.ts source/utils/execution/teamExecutor.test.ts` = `27 passed`，`npm run build:ts -- --pretty false` 通过，`git diff --check` 通过。当前 Kimi 审计里剩余未动的仅是 `teamExecutor` synthetic tools 前移到路由层这一条非阻断整洁度优化，继续保留在 backlog，不把它误判成 runtime blocker
